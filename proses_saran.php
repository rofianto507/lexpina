...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Kategori harus salah satu dari pilihan yang valid
    $kategori_valid = ['Penambahan Peraturan', 'Performa Website', 'Pembayaran'];
    if (!in_array($kategori, $kategori_valid, true)) {
        $kategori = 'Performa Website';
    }
    ...
    if (!empty($judul) && !empty($konten)) {
        ...
        try {
            // Simpan ke database
            $stmt = $pdo->prepare("INSERT INTO sarans (user_id, judul, kategori, konten, status) VALUES (:user_id, :judul, :kategori, :konten, 0)");
