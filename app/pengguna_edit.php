 ...
 if(empty($_SESSION['csrf_token'])) {
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (
        ...
        header("Location: pengguna_edit?id=$id_user&status=csrf_failed");
        exit;
    }
  }
  
  $id_user = isset($_GET['id']) ? intval($_GET['id']) : 0;
  if(!$id_user){ header("Location: pengguna"); exit; }

  // Ambil data user
  $quser = $pdo->prepare("SELECT * FROM users WHERE id=? LIMIT 1"); 
  $quser->execute([$id_user]);
  $data = $quser->fetch(PDO::FETCH_ASSOC);
  if(!$data) { header("Location: pengguna"); exit; }

  // Untuk akses drop-down
  $aksesList = [
    'POLDA','POLRES','POLSEK',
    'DITNARKOBA','DITRESKRIMSUS','DITRESKRIMUM','DITINTELKAM','DITPOLAIRUD',
     'DITLANTAS','DITSAMAPTA','DITSABHARA','PPA','PPO','SPKT'
  ];

  // Ambil polres dan polsek milik user jika ada
  $polres_id = 0; $polsek_id=0;
  $stmt = $pdo->prepare("SELECT id FROM polress WHERE user_id=?"); $stmt->execute([$id_user]);
  if($row = $stmt->fetch(PDO::FETCH_ASSOC)){ $polres_id = $row['id']; }
  $stmt = $pdo->prepare("SELECT id, polres_id FROM polseks WHERE user_id=?"); $stmt->execute([$id_user]);
  if($row = $stmt->fetch(PDO::FETCH_ASSOC)){ $polsek_id = $row['id']; $polsek_polres_id = $row['polres_id']; }
  ...
  if($_SERVER['REQUEST_METHOD']=='POST' && isset($_POST['edit_id'])){
    ...
    if(empty($error) && $nama_inp && $username_inp && $akses_inp){
      ...
      $pdo->prepare("UPDATE polress SET user_id=NULL WHERE user_id=?")->execute([$edit_id]);
      $pdo->prepare("UPDATE polseks SET user_id=NULL WHERE user_id=?")->execute([$edit_id]);

      // Jika akses POLRES dan polres_id valid, update polress.user_id
      if($akses_inp=="POLRES" && $polres_id_inp) {
        $pdo->prepare("UPDATE polress SET user_id=? WHERE id=?")->execute([$edit_id, $polres_id_inp]);
      }
      // Jika akses POLSEK, update polseks.user_id
      if($akses_inp=="POLSEK" && $polsek_id_inp) {
        $pdo->prepare("UPDATE polseks SET user_id=? WHERE id=?")->execute([$edit_id, $polsek_id_inp]);
      }

      header("Location: pengguna?status=edit_sukses"); exit;
    }
  }
...
<head>
  <meta charset="utf-8">
  <title>Edit Pengguna | Peta Digital</title>
...
<body>
              ...
              <div class="card-header bg-light">
                <div class="row flex-between-end">
                  <div class="col-auto align-self-center">
                    <h5 class="fs-0 mb-0"><span class="fa fa-edit me-2 fs-0"></span> Edit Data Pengguna</h5>
                  </div>
                  <div class="col-auto ms-auto">
                    <a href="pengguna" class="btn btn-falcon-default btn-sm">Kembali ke Data Pengguna</a>
                  </div>
                </div>
              </div>
              <div class="card-body">
              ...
              <?php endif; ?>
                <form method="POST" enctype="multipart/form-data">
                  ...
                  <div class="mb-3">
                    ...
                    <?php if($data['foto']): ?>
                      <br><img src="../public/upload/pengguna/<?=$data['foto']?>" class="preview-foto-user" alt="Foto Pengguna">
                    <?php endif; ?>
          ...
          <div class="col-lg-4">
            <div class="card mb-3">
              ...
              <div class="card-body">
                ...
                <p><strong>Akses lain (DITNARKOBA, DITRESKRIMSUS, DITRESKRIMUM, DITINTELKAM, DITPOLAIRUD):</strong> Tidak perlu memilih Polres/Polsek. User ini bisa mengelola data di semua wilayah.</p>
              </div>
            </div>
        </div>
  ...
  <script src="../assets/js/pengguna-edit.js"></script>
