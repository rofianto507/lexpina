...
$q = $pdo->prepare("
  ...
  WHERE co.polres_id = ? AND co.status=1 AND co.state!='SELESAI' $whereKategori $whereTahun
  GROUP BY kc.id
");
$q->execute([$polres_id]);
...
$features = [];
$qKec = $pdo->prepare("SELECT kecamatans.id, kecamatans.nama, kecamatans.geom FROM kecamatans join kabupatens ON kabupatens.id = kecamatans.kabupaten_id WHERE kabupatens.polres_id=? AND kecamatans.status=1");
...
$sumberSql = "SELECT DISTINCT s.nama 
 ...
 LEFT JOIN sumbers s ON co.sumber_id = s.id
 WHERE co.polres_id = ? AND co.status=1 AND co.sumber_id IS NOT NULL AND co.state!='SELESAI' $whereKategori $whereTahun";
$stmtSumber = $pdo->prepare($sumberSql);
$stmtSumber->execute([$polres_id]);
