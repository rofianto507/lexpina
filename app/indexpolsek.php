...
<html lang="en-US" dir="ltr">
  ...
  <head>
    ...
    <!--    Document Title-->
    <!-- ===============================================-->
    <title>Dashboard Polsek | Peta Digital</title>
...
//------------------------------------------
  ...
  function showDefaultMap() { 
    // Fungsi untuk menampilkan peta default kosong
      ...
      let url = 'umum_kecamatan_polsek_geojson.php?polsek_id=' + <?php echo $polsek_id; ?>;
      fetch(url)
      .then(response => response.json())
      .then(function(geojson) {
        window.kecamatanLayer = L.geoJSON(geojson, {
          ...
          onEachFeature: function(feature, layer) {
            ...
            var kecamatan_id = feature.properties.id;
            ...
            var html = `<b>${feature.properties.nama}</b><br>
              <br>
              <button class="btn btn-zoom-kecamatan btn-primary btn-sm me-1 mb-1" data-kecid="${kecamatan_id}">Lihat Detail</button>`;
        ...
        if(window.kecamatanLayer && window.kecamatanLayer.getBounds && window.kecamatanLayer.getBounds().isValid()){
            map2.fitBounds(window.kecamatanLayer.getBounds(), {maxZoom: 14}); // maxZoom bisa kamu adjust!
        }
  ...
  function showKriminalitasMap(filterKategoriIds = [], tahun = new Date().getFullYear()) {
    ...
    let polsek_id = <?php echo $_SESSION['polsek_id']; ?>;
    let endpoint = 'kriminalitas_polsek_geojson.php';
    let params = [];
    if (filterKategoriIds.length) params.push('kategori=' + filterKategoriIds.join(','));
    if (tahun) params.push('tahun=' + tahun);
    if (polsek_id) params.push('polsek_id=' + polsek_id);
    if(params.length) endpoint += '?' + params.join('&');
    console.log("fetching Kriminalitas map with endpoint: ", endpoint);
    fetch(endpoint)
      .then(res => res.json())
      .then(function(geojson){
        window.kabupatenLayer = L.geoJSON(geojson, {
      ...
      let markerParams = [];
      if (polsek_id) markerParams.push('polsek_id=' + polsek_id);
      ...
      showTableWilayah('kriminalitas', 'polsek', polsek_id);
  }
