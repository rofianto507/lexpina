 ...
 if(empty($_SESSION['csrf_token'])) {
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (
        ...
        header("Location: jenis-tkp?status=csrf_failed");
        exit;
    }
  }
  ...
  if($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['edit_id'], $_POST['edit_nama'])) {
    ...
    if($edit_id && $edit_nama != "") {
      $stmtUpdate = $pdo->prepare("UPDATE jenis_tkps SET nama=:nama, updated_at=NOW() WHERE id=:id");
      $stmtUpdate->execute([':nama'=>$edit_nama, ':id'=>$edit_id]);
      header("Location: jenis-tkp?status=edit_sukses");
      exit;
    }
  }
  // Handle proses Hapus
  if($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['hapus_id'])) {
    ...
    if($hapus_id) {
      $stmtDel = $pdo->prepare("UPDATE jenis_tkps SET status=0 WHERE id=:id");
      $stmtDel->execute([':id'=>$hapus_id]);
      header("Location: jenis-tkp?status=hapus_sukses");
      exit;
    }
  }
?>
...
<html lang="en-US" dir="ltr">
  <head>
    ...
    <title>Jenis TKP | Peta Digital</title>
  ...
  <body>
    <main class="main" id="top">
      <div class="container-fluid" data-layout="container">
        ...
        <div class="content">
            ...
            <div class="card-header">
              <div class="row flex-between-end">
                <div class="col-auto align-self-center">
                  <h5 class="fs-0 mb-0"><span class="fa fa-bookmark me-2 fs-0"></span> Jenis TKP</h5>
                </div>
            ...
            <div class="card-body bg-light">
              <div class="table-responsive">
                <table id="kategoriTable" class="display table table-striped table-bordered table-sm"  >
                  ...
                  <?php
                    $no=1;
                    $stmt = $pdo->query("SELECT * FROM jenis_tkps WHERE status=1 ORDER BY id ASC");
          ...
          <!-- Modal Tambah Data Jenis TKP -->
          <div class="modal fade" id="modalTambahKategori" tabindex="-1" aria-labelledby="modalTambahKategoriLabel" aria-hidden="true">
            <div class="modal-dialog">
              <form method="POST" id="formTambahKategori">
                <div class="modal-content">
                  <div class="modal-header">
                    <h5 class="modal-title" id="modalTambahKategoriLabel">Tambah Jenis TKP</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Tutup"></button>
                  </div>
                  <div class="modal-body">
                    <div class="mb-3">
                      <label for="namaKategori" class="form-label">Nama Jenis TKP</label>
          ...
          <div class="modal fade" id="modalEditKategori" tabindex="-1" aria-labelledby="modalEditKategoriLabel" aria-hidden="true">
            <div class="modal-dialog">
              <form method="POST" id="formEditKategori">
                ...
                <div class="modal-content">
                  <div class="modal-header">
                    <h5 class="modal-title" id="modalEditKategoriLabel">Edit Jenis TKP</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                  </div>
                  <div class="modal-body">
                    <div class="mb-3">
                      <label for="edit_nama" class="form-label">Nama Jenis TKP</label>
    ...
    <script src="../assets/js/jenis-tkp.js"></script>
