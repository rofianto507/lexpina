...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        header("Location: pengguna?msg=csrf_failed");
        exit;
    }
    ...
    try {
        ...
        if ($action == 'edit' && !empty($_POST['edit_id'])) {
            ...
            $stmt = $pdo->prepare("UPDATE users SET nama = ?, status = ? WHERE id = ? AND akses = 'PENGGUNA'");
            $stmt->execute([$edit_nama, $edit_status, $edit_id]);

            header("Location: pengguna?msg=sukses_edit");
            exit;
        }

        // --- 2. PROSES HAPUS (HARD DELETE) ---
        // Karena ini data user, kita gunakan Delete permanen agar username/email bisa dipakai mendaftar lagi jika dibutuhkan
        if ($action == 'hapus' && !empty($_POST['hapus_id'])) {
            $hapus_id = (int)$_POST['hapus_id'];
            
            // Opsional: Hapus foto profil dari server jika ada
            // $stmt_foto = $pdo->prepare("SELECT foto FROM users WHERE id = ?");
            // $stmt_foto->execute([$hapus_id]);
            // $foto_lama = $stmt_foto->fetchColumn();
            // if($foto_lama && file_exists('../public/upload/users/'.$foto_lama)) unlink('../public/upload/users/'.$foto_lama);

            $stmt = $pdo->prepare("DELETE FROM users WHERE id = ? AND akses = 'PENGGUNA'");
            $stmt->execute([$hapus_id]);
            
            header("Location: pengguna?msg=sukses_hapus");
            exit;
        }
        ...
        <div class="content">
          ...
          <?php if(isset($_GET['msg'])): ?>
            <div class="alert alert-success alert-dismissible fade show mt-3" role="alert">
                <?php
                if($_GET['msg'] == 'sukses_edit') echo "Data pengguna berhasil diperbarui.";
                elseif($_GET['msg'] == 'sukses_hapus') echo "Akun pengguna berhasil dihapus secara permanen.";
            ...
            <div class="card-header">
              <div class="row flex-between-end">
                <div class="col-auto align-self-center">
                  <h5 class="fs-0 mb-0"><span class="fa fa-users me-2 fs-0 text-primary"></span>Data Akun: Pengguna (Gratis)</h5>
                </div>
              </div>
            </div>
            <div class="card-body bg-light">
              <div class="table-responsive">
                <table id="penggunaTable" class="display table table-striped table-bordered table-sm" style="width:100%">
                  ...
                  <tbody>
                  <?php
                    // Filter khusus untuk akses = PENGGUNA
                    $stmt = $pdo->query("SELECT * FROM users WHERE akses = 'PENGGUNA' ORDER BY created_at DESC");
                    $no = 1;
                    ...
                    while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                      ...
                      echo "<tr>
                        ...
                        <td class='text-center align-middle'>
                            <img src='".htmlspecialchars($avatar)."' referrerpolicy='no-referrer' class='rounded-circle' width='40' height='40' style='object-fit:cover;' alt='Avatar'>
                        </td>
                        <td class='align-middle fw-semi-bold'>".htmlspecialchars($row['nama'])."</td>
                        <td class='align-middle'>".htmlspecialchars($row['username'])."</td>
                        <td class='align-middle'>".$tgl_daftar."</td>
                ...
                <div class="modal-content">
                  ...
                  <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <small class="text-muted">Username tidak disarankan untuk diubah jika terhubung dengan integrasi Google Login.</small>
                    </div>
                    <div class="mb-3">
              ...
              <form method="POST" action="">
                ...
                <div class="modal-content">
                  ...
                  <div class="modal-body text-center">
                    <p class="mb-0">Tindakan ini bersifat permanen. Yakin ingin menghapus akun <br><strong id="label_hapus_nama" class="text-danger"></strong>?</p>
                  </div>
...
<script src="../vendors/jquery/jquery-3.7.0.min.js"></script>
    ...
    $(document).ready(function() {
      ...
      $('#penggunaTable').DataTable({
        "autoWidth": false,
        "order": [[ 4, "desc" ]] // Urutkan berdasarkan tanggal daftar terbaru
      });
