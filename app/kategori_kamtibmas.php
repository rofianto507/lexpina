...
if(!isset($_SESSION["id"]) || !isset($_SESSION["nama"])) {
 ...
 if(empty($_SESSION['csrf_token'])) {
  ...
  $_SESSION["menu"]="kategori-kamtibmas";
  $menu=$_SESSION["menu"];
  $nama=$_SESSION["nama"];
  $id=$_SESSION["id"];
  ...
  $last_login=$_SESSION["last_login"];
   if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (
        ...
        header("Location: kategori-kamtibmas?status=csrf_failed");
        exit;
    }
  }
  if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['namaKategori'])) {
      ...
      if($namaKategori != "") {
          $stmtInsert = $pdo->prepare("INSERT INTO kamtibmas_kategoris (nama, status) VALUES (:nama, 1)");
          $stmtInsert->bindParam(':nama', $namaKategori);
          $stmtInsert->execute();
          // Redirect agar refresh page dan menghilangkan POST (juga agar tabel otomatis update)
          header("Location: kategori-kamtibmas?status=sukses");
          exit;
      }
  }
  ...
  if($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['edit_id'], $_POST['edit_nama'])) {
    ...
    if($edit_id && $edit_nama != "") {
      $stmtUpdate = $pdo->prepare("UPDATE kamtibmas_kategoris SET nama=:nama, updated_at=NOW() WHERE id=:id");
      $stmtUpdate->execute([':nama'=>$edit_nama, ':id'=>$edit_id]);
      header("Location: kategori-kamtibmas?status=edit_sukses");
      exit;
    }
  }
  // Handle proses Hapus
  if($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['hapus_id'])) {
    ...
    if($hapus_id) {
      $stmtDel = $pdo->prepare("UPDATE kamtibmas_kategoris SET status=0 WHERE id=:id");
      $stmtDel->execute([':id'=>$hapus_id]);
      header("Location: kategori-kamtibmas?status=hapus_sukses");
      exit;
    }
  }
?>
...
<html lang="en-US" dir="ltr">
  <head>
    ...
    <title>Kategori Kamtibmas | Peta Digital</title>
  ...
  <body>
    <main class="main" id="top">
      <div class="container-fluid" data-layout="container">
        ...
        <div class="content">
            ...
            <div class="card-header">
              <div class="row flex-between-end">
                <div class="col-auto align-self-center">
                  <h5 class="fs-0 mb-0"><span class="fa fa-bookmark me-2 fs-0"></span> Kategori Kamtibmas</h5>
                </div>
            ...
            <div class="card-body bg-light">
              <div class="table-responsive">
                <table id="kategoriTable" class="display table table-striped table-bordered table-sm" >
                  ...
                  <?php
                    $no=1;
                    $stmt = $pdo->query("SELECT * FROM kamtibmas_kategoris WHERE status=1 ORDER BY id ASC");
          ...
          <!-- Modal Tambah Data Kategori kamtibmas -->
          <div class="modal fade" id="modalTambahKategori" tabindex="-1" aria-labelledby="modalTambahKategoriLabel" aria-hidden="true">
            <div class="modal-dialog">
              <form method="POST" id="formTambahKategori">
                <div class="modal-content">
                  <div class="modal-header">
                    <h5 class="modal-title" id="modalTambahKategoriLabel">Tambah Kategori Kamtibmas</h5>
          ...
          <div class="modal fade" id="modalEditKategori" tabindex="-1" aria-labelledby="modalEditKategoriLabel" aria-hidden="true">
            <div class="modal-dialog">
              <form method="POST" id="formEditKategori">
                ...
                <div class="modal-content">
                  <div class="modal-header">
                    <h5 class="modal-title" id="modalEditKategoriLabel">Edit Kategori Kamtibmas</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                  </div>
                  <div class="modal-body">
                    <div class="mb-3">
                      <label for="edit_nama" class="form-label">Nama Kategori Kamtibmas</label>
   ...
   <script src="../vendors/jquery/jquery-3.7.0.min.js"></script>
    ...
    <script src="../assets/js/kategori-kamtibmas.js"></script>
